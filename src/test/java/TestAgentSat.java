...
public class TestAgentSat {
	...
	// create player grid and place ships. also create computer attack board
	// and the corresponding influence map
	 ...
	 * 
	 * System.out.println("size is " + Store.size()); // if (Store.size()>
	 * 1) //m = (InfluenceMap)Store.getBestMap(); //else // m =
	 * Store.getMap(0); if (m ==null) System.out.println("m is null");
	 * 
	 * //m =myStore.getMap((myStore.size()-1)); /* //sums the contents of
	 * all the previous maps into one map for(int l = 0; l <Store.size();
	 * l++) { myMap = (InfluenceMap)Store.getMap(l);
	 * 
	...
	/*
	 * Configuration 3 * playerHomeBoard.addAir(0,0, 1);
	 * playerHomeBoard.addBattle(0,3, 1); playerHomeBoard.addSub(0,5, 1);
	 * playerHomeBoard.addMine(0,7,1); playerHomeBoard.addDest(0,9, 1); /*
	 */

	/*
	 * Configuration 1/* playerHomeBoard.addAir(0,0, 1);
	 * playerHomeBoard.addBattle(0,1, 1); playerHomeBoard.addSub(0,2, 1);
	 * playerHomeBoard.addMine(0,3,1); playerHomeBoard.addDest(0,4, 1); /*
	 */
	/*
	 * Configuration 2/* playerHomeBoard.addAir(0,0, 0);
	 * playerHomeBoard.addBattle(1,0, 0); playerHomeBoard.addSub(2,0, 0);
	 * playerHomeBoard.addDest(3,0, 0); playerHomeBoard.addMine(4,0,0); /*
	 */
	/*
	 * Random* playerHomeBoard.addAir(3,5, 1);
	 * playerHomeBoard.addBattle(0,3, 1); playerHomeBoard.addSub(1,5, 0);
	 * playerHomeBoard.addMine(4,6,1); playerHomeBoard.addDest(8,0, 0); /*
	 */
...

	{
		...
		// this section decides if there are any hotspots, if there are
		// it will set i and j to them
		// otherwise it will pick numbers at random.
		// int maxVal = m.getMaxHotspotVal();
		// do random search

		System.out.println("Value of highest hs is "
			+ m.getMaxHotspotVal());
		System.out
			.println("Number of hs is " + m.getNumberOfHotspots());

		// if there is only one hotspot, and it has not already been
		// shot at
		if (m.getNumberOfHotspots() == 1) {
			...
			i = m.getHotspotI();
			j = m.getHotspotJ();
			System.out.println("Target single hotspot at " + i
				+ " " + j);
		    }
		    ...
		    /*
		     * i=m.getHotspotI(); j=m.getHotspotJ();
		     * 
		     * System.out.println("Targeting single hotspot");
		     * 
		     * while(computerAttackBoard.getGridVal(i,j) == 8 ||
		     * computerAttackBoard.getGridVal(i,j) == 1) {
		     * System.out.println("i = "+ i + " j=" + j); i =
		     * gen.rand(10); j = gen.rand(10); }
		     */
		}
		...
		if (m.getNumberOfHotspots() > 1) {
			...
			int loop = 0;
			while (computerAttackBoard.isValidPlaceForAShip(i, j)
				&& !noneFound) {
			    if (loop == 100)
				noneFound = true;
		...
		if (sqrVal == 1) {
		     ...
		     * while(computerAttackBoard.getGridVal(i,j) !=0) {
		     * 
		     * //select a downward right diagonal if(lasti+1 != 10 &&
		     * (lastj+1) != 10) { System.out.println("right diagonal");
		     * i = lasti+1; j = lastj+1; } else
		     * 
		     * System.out.println("random gen"); i = gen.rand(10); j =
		     * gen.rand(10); }
		     */
		    // agentTurn = false;
		    m.miss(i, j);
		...
		/*
		 * for(int c = 0; c<100000;c++) {//waste time }
		 */
	    }
	    turns++;
	    turns2++;
		...
		m.clearAll();
		System.out
			.println("20 or more turns without a hit, clearing Influence map");
		turns2 = 0;
	    }
	    /*
	     * if (turns >= 30 && misses == 15) { m.clearAll(); }
	     */
	...
	// moderating the map reduces it's 888 hit values to smaller that are
	// incremented ones
	...
	/*
	 * System.out.println(m.toString()); //store.add(m);
	 * 
	 * 
	 * System.out.println("Summed moderating map");
	 * System.out.println(moderatingMap.toString());
	 * 
	 * // moderatingMap.addMap(m); // moderatingMap.addMap(m);
	 * //moderatingMap.addMap(m); /*
	 ...
	 * 
	 * //Initialise a new Map object //InfluenceMap myMap = new
	 * InfluenceMap(); //InfluenceMap currentMap = null;
	 * 
	 * 
	 * //checks if datastore file exists if(f.exists()) { InfluenceMap
	 * blankMap = new InfluenceMap();
	 * 
	 * ObjectInputStream in = new ObjectInputStream(new FileInputStream(f));
	 * 
	 * System.out.println("file exists");
	 * 
	 * myStore = (MapStore)in.readObject(); //currentMap = new
	 * InfluenceMap();
	 * 
	 * 
	 * 
	 ...
	 * System.out.println(moderatingMap.toString()); /*
	 * //m.addMap(blankMap); blankMap.addMap(m);
	 ...
	 * +"\n with " +turns + " Turns"); moderatingMap.setTurns(turns);
	 * myStore.add(moderatingMap);
	 * 
	 * in.close();
	 * 
	 * ObjectOutputStream out = new ObjectOutputStream(new
	 * FileOutputStream(f)); out.writeObject(myStore); out.close(); }
	 * 
	 * else if(!f.exists()) { myStore = new MapStore();
	 * 
	 * //myGrid2.get(1,2);
	 * 
	 * 
	 ...
	 * 
	 * System.out.println("New file created the number of maps stored is = "
	 * + myStore.size());
	 * 
	 * ObjectOutputStream out = new ObjectOutputStream(new
	 * FileOutputStream(f)); System.out.println("The map stored is \n" +
	 * moderatingMap.toString()); out.writeObject(myStore); out.close(); }
	 */

	// System.out.println("The number of maps stored is = " +
	// myStore.size());
	System.out.println("Game over man, number of turns was " + turns);
